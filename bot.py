# bot.py (Version 21 - The Final Polish: Auto-Cleanup of Previous Request)
...
# --- Deletion Function (for 1-minute timer) ---
async def schedule_message_deletion(context: ContextTypes.DEFAULT_TYPE, chat_id: int, bot_message_id: int, user_message_id: int, user_name: str, delay: int = 60):
    ...
    try:
        # Also clear the stored IDs when the timer deletion runs
        if 'last_message_ids' in context.user_data:
            del context.user_data['last_message_ids']
            
        await context.bot.delete_message(chat_id=chat_id, message_id=bot_message_id)
        await context.bot.delete_message(chat_id=chat_id, message_id=user_message_id)
        logger.info(f"Auto-deleted messages via timer in chat {chat_id}.")
        ...
        logger.warning(f"Could not delete messages via timer in chat {chat_id}: {e}")
...
async def update_index(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    post, caption = update.channel_post, post.caption or post.text
    ...
    if title_match and lang_match:
        title, lang, index_key = title_match.group(1).strip().lower(), lang_match.group(1).strip().lower(), f"{title}_{lang}"
...
def create_url_buttons_from_caption(caption: str) -> (str, InlineKeyboardMarkup | None):
    lines, buttons, cleaned_lines, url_regex = caption.split('\n'), [], [], r'https?://[^\s]+'
    for i, line in enumerate(lines):
        if re.search(url_regex, line):
            if i > 0 and not re.search(url_regex, lines[i-1]):
                for url in re.findall(url_regex, line): buttons.append([InlineKeyboardButton(lines[i-1].strip(), url=url)])
...
# --- MODIFIED: Main search function with auto-cleanup of previous search ---
async def search_media(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Before we search, check if there are old message IDs to delete
    if 'last_message_ids' in context.user_data:
        logger.info("Cleaning up previous search messages.")
        for msg_id in context.user_data['last_message_ids']:
            try:
                await context.bot.delete_message(chat_id=user.id, message_id=msg_id)
            except Exception:
                pass # Ignore errors if messages were already deleted
        # Clean up the list from memory
        del context.user_data['last_message_ids']
    ...
    if user_lang_code:
        ...
        if movie_data:
            ...
            try:
                ...
                logger.error(f"Failed to re-send message from index: {e}"); await update.message.reply_text("I found the movie, but couldn't send it.")
    ...
    # --- Step 3: Schedule deletion & store message IDs ---
    if sent_message:
        # Store the new message IDs for the *next* auto-cleanup
        context.user_data['last_message_ids'] = [user_message_id, sent_message.message_id]
        
        # Add the deletion warning and schedule the 1-minute timer
        deletion_warning = "\n\n\n*⚠️ This message & your request will automatically delete in 1 minute!*"
        try:
            new_caption = sent_message.caption + deletion_warning if sent_message.caption else deletion_warning
            await context.bot.edit_message_caption(chat_id=sent_message.chat.id, message_id=sent_message.message_id, caption=new_caption, parse_mode='Markdown', reply_markup=sent_message.reply_markup)
        except Exception as e:
            logger.info(f"Could not add deletion warning (likely a text-only message): {e}")

        # Schedule the 1-minute timer deletion as a fallback
        asyncio.create_task(schedule_message_deletion(context, sent_message.chat_id, sent_message.message_id, user_message_id, user.first_name, 60))
    else:
        # --- Step 4: If nothing was found anywhere ---
...
async def search_tmdb(query: str, region: str | None = None, lang_code: str | None = None) -> dict | None:
    try:
        ...
        response, search_data = requests.get(search_url, headers=headers), response.json()
        if not search_data.get("results"): return None
        found_movie_id = None
        if lang_code:
            for movie in search_data["results"]:
                if movie.get('original_language') == lang_code: found_movie_id = movie['id']; break
        else: found_movie_id = search_data["results"][0]["id"]
        if not found_movie_id: return None
        details_url, details_response = f"https://api.themoviedb.org/3/movie/{found_movie_id}?language=en-US", requests.get(details_url, headers=headers)
...
async def error_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None: logger.warning('Update "%s" caused error "%s"', update, context.error)
def main() -> None:
    if not all([TELEGRAM_TOKEN, TMDB_API_KEY, PRIVATE_CHANNEL_ID]): logger.error("One or more required environment variables are missing!"); return
    ...
    PORT, APP_NAME = int(os.environ.get('PORT', 8443)), os.environ.get("RENDER_EXTERNAL_HOSTNAME")
    if not APP_NAME: logger.error("RENDER_EXTERNAL_HOSTNAME not set!"); return
