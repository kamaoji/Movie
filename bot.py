# bot.py (Version 20 - Final Polish: Private Channel First)
...
async def update_index(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if title_match and lang_match:
        ...
        try:
            ...
            pass # Harmless warning, we just ignore it now
...
async def search_media(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # --- Step 2: Try TMDB as a fallback (only if not found in private channel) ---
    if not sent_message:
    ...
    # --- Step 3: Schedule deletion if any message was sent ---
    if sent_message:
