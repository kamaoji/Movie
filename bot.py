# bot.py (Version 6 - Highlighting IMDb Data)
...
# --- Helper function to search using TMDB (Unchanged) ---
...
# --- MODIFIED: Helper function to search using OMDb to highlight IMDb data ---
async def search_omdb(query: str) -> dict | None:
    ...
    try:
        ...
        # Prepare a standardized data dictionary, highlighting IMDb source
        return {
            "source": "IMDb",  # We now label the source as IMDb
            "title": data.get("Title", "N/A"),
            "rating": f"{data.get('imdbRating', 'N/A')} / 10", # Specify this is the IMDb rating
...
# --- Main handler that orchestrates the search (Unchanged) ---
async def search_media(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # The source is now dynamic (TMDB or IMDb)
    caption = (
        f"🎬 *{media_data['title']}*\n\n"
